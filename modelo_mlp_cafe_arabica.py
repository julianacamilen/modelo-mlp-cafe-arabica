...
def build_mlp_model(input_shape):
    model = Sequential([
        Dense(512, activation='relu', input_shape=(window_size,)),
        Dropout(0.2),
...
# Para exportar as previsões, a fim de comparar com outros modelos
# Vamos considerar o mesmo dataset da plotagem das previsões
valid = df[train_size:].copy()
valid["Previsões"] = scaler.inverse_transform(y_pred.reshape(-1, 1))

# Seleção de colunas
valid_export = valid[["data", "preco_reais", "Previsões"]]
valid_export.columns = ["date", "y_test", "pred_mlp"]

# Exportar como CSV
valid_export.to_csv("predicoes_mlp.csv", index=False)
print(valid_export)